...
const { Client, GatewayIntentBits, Collection, MembershipScreeningFieldType, ClientUser, User, time, GuildChannel, GuildManager, MessageManager, GuildMemberManager, GuildBanManager, GuildBan, GuildStickerManager, PermissionsBitField, PermissionOverwriteManager, MessageFlagsBitField, GuildMemberRoleManager, GuildMember, MessageReaction, SlashCommandBuilder, RoleManager, ChannelManager, ChannelFlagsBitField, ChannelFlags} = require('discord.js');
...
const { ekdha, token } = require('./config.json');
...
tpttlrks *=0
    ...
    console.log(interaction.member.voicechannelid)
console.log(interaction.channelId)
console.log(interaction.guildId)
console.log('노래')
const player = createAudioPlayer()
const resource = createAudioResource('./music/cherry.mp3')
...
player.on(AudioPlayerStatus.Idle, () => {
  connection.destroy();
    ...
    let timecc = 0
    await interaction.reply({ files: [`./cherry/Frame${timecc}.jpg`] })
    var timer = setInterval(() => {
        timecc += 10
        console.log(timecc)
        interaction.editReply({ files: [`./cherry/Frame${timecc}.jpg`] })
      .catch(console.warn = () => {});
        if (timecc === 3244) {
            clearInterval(timer)
            .catch(console.warn = () => {});
...
if(message.content.startsWith(`!타이머`)) { // If the message content is "!ping"
  ...
  const tic = await message.reply({ files: ['./roll/Preview'] })
  var timer = setInterval(() => {
      timecc ++
      tic.edit({ files: ['./roll/Preview'] })
      .catch(console.warn = () => {});
...
if(message.content.startsWith(`!얻기`)) {
const channel = message.guild.channels.cache.get('1014398720240005162')
console.log(channel)
